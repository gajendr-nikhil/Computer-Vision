...
def part_1a():
    ...
    shift_r2_blurred = cv2.GaussianBlur(shift_r2, (7, 7), 2)
    ...
    u, v = ps6.optic_flow_lk(shift_0_blurred, shift_r2_blurred, k_size, k_type, sigma)
    ...
    shift_0_blurred = cv2.GaussianBlur(shift_0, (7, 7), 2)
    shift_r5_u5_blurred = cv2.GaussianBlur(shift_r5_u5, (7, 7), 2)

    k_size = 8  # Select a kernel size
    k_type = "uniform"  # Select a kernel type
    sigma = 1  # Select a sigma value if you are using a gaussian kernel
    u, v = ps6.optic_flow_lk(shift_0_blurred, shift_r5_u5_blurred, k_size, k_type, sigma)
...
def part_2a_2b(save_imgs=True, levels=4):
...
def part_3a_1():
    yos_img_01, yos_img_01_g_pyr, yos_img_01_l_pyr = part_2a_2b(False, 10)

    yos_img_02 = cv2.imread(os.path.join(input_dir, 'DataSeq1', 'yos_img_02.jpg'), 0) / 255.

    levels = 10  # Define the number of levels to build the gaussian pyramid
    yos_img_02_g_pyr = ps6.gaussian_pyramid(yos_img_02, levels)

    level_id = 4  # Select the level number (or id) you wish to use
    k_size = 8  # Select a kernel size
    k_type = "uniform"  # Select a kernel type
    sigma = 2  # Select a sigma value if you are using a gaussian kernel
    ...
    scale = 2  # define a scale value
    stride = 12  # define a stride value
...
def part_4a():
    ...
    levels = 15  # Define the number of levels
    k_size = 8  # Select a kernel size
    k_type = "uniform"  # Select a kernel type
    sigma = 1  # Select a sigma value if you are using a gaussian kernel
    interpolation = cv2.INTER_CUBIC  # Select an interpolation method (see cv2.remap)
...
def part_4c():
    ...
    dtsq2_01 = cv2.GaussianBlur(dtsq2_01, (7, 7), 2)
    dtsq2_02 = cv2.GaussianBlur(dtsq2_02, (7, 7), 2)
...
def part_5a():
    ...
    juggle_01 = cv2.cvtColor(cv2.imread(os.path.join(input_dir, 'Juggle', '0.png')), cv2.COLOR_BGR2GRAY) / 255.
    juggle_02 = cv2.cvtColor(cv2.imread(os.path.join(input_dir, 'Juggle', '1.png')), cv2.COLOR_BGR2GRAY) / 255.
    juggle_03 = cv2.cvtColor(cv2.imread(os.path.join(input_dir, 'Juggle', '2.png')), cv2.COLOR_BGR2GRAY) / 255.
